...
"""
from adaptive_collapsing_permute import *
import argparse
import os
 
def run(output_file, alpha, nperms):
    smps = []
    spls = []
    regs = []
    cwd = os.getcwd()
    ...
    with open(output_file, 'w') as outfile:
        for smp, spl, reg in zip(smps, spls, regs):
            results = collapse(smp, spl, reg, nperms)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nperms', type=int, help='Specify number of perms')
    args = parser.parse_args()
    run(args.output_file, args.alpha, args.nperms)
